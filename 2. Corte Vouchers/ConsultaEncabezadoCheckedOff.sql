...
from (
  ...
  left join (
    SELECT tipooperacion, agrupador, sum(importe) as importe,
      fecha, idcaja, caja, statusDetalle, count(statusDetalle) CStatus
    FROM (
      select 0 as Movimiento,
        case
        ...
        if(V.status = 1, 'VENTA', 'CANCELACION') StatusDetalle
      from voucher V
  ...
  select 'Total' AS descripcion, c1.tipo, ifnull(sum(m.importe), 0) as importe,
    ...
    idcaja as idcaja, idcaja as caja, 0 as CStatus, 3 as agrupador, 100 as orden
  from cortevoucherorden c1
  left join (
    SELECT tipooperacion, agrupador,
      ifnull(sum(importe), 0) as importe,
      fecha, idcaja, caja, statusDetalle,
      count(statusDetalle) CStatus
    FROM (
      select 0 as Movimiento,
        case
          when op.descripcion = 'AMERICAN EXPRESS' then 'AMERICAN EXPRESS'
        ...
        ifnull(V.IMPORTE, 0) as importe,
        v.fecha as fecha, c.idcaja, caja,
        if(V.status = 1, 'VENTA', 'CANCELACION') Status,
        if(V.status = 1, 'VENTA', 'CANCELACION') StatusDetalle
      from voucher V
      inner join banco b on v.idbanco = b.idbanco
